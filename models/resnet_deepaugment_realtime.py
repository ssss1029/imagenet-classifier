...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, block_distort_prob=0.0):
        identity = x

        out = self.conv1(x)
        out = random_distortion(out, p=block_distort_prob)
        out = self.bn1(out)
        out = self.relu(out)
...
def random_distortion(x, p):
    if random.random() < p:
        dist = random.choice(distortion_functions)
        return dist(x)
    else:
        return x
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, block_distort_prob=0.0):
        ...
        for block in self.layer1:
            x = block(x, block_distort_prob=block_distort_prob)
    ...
    def forward(self, x, block_distort_prob=0.0):
        return self._forward_impl(x, block_distort_prob=block_distort_prob)
