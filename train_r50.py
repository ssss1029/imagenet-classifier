...
def main_worker(gpu, args):
    ...
    val_loader_imagenet_r = torch.utils.data.DataLoader(
        datasets.ImageFolder("/data/sauravkadavath/imagenet_r", transforms.Compose([
            transforms.Resize(256),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
            normalize,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss_T, acc1_T, acc5_T = train(train_loader, model, criterion, optimizer, scheduler, epoch, args)

        # evaluate on validation set
        acc1_V, acc5_V = validate(val_loader, model, criterion, args)
...
def validate_imagenet_r(val_loader, model, criterion, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
