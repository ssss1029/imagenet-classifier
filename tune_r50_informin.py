...
def main_worker(gpu, args):
    ...
    optimizer = torch.optim.SGD(classifier_head.parameters(), args.lr,
    ...
    if os.path.isfile(args.resume):
        ...
        tarball = torch.load(args.resume)

        if 'model' in tarball:
            checkpoint = tarball['model']
        elif 'state_dict' in tarball:
            checkpoint = tarball['state_dict']
        else:
        ...
        for key, value in checkpoint.items():
            new_key = key.split(".")
            new_key = [part for part in new_key if part != "encoder"]
        ...
        if 'epoch' in tarball:
            print("Restoring epoch to {0}".format(tarball['epoch']))
            args.start_epoch = tarball['epoch']
        
        if 'classifier_state_dict' in tarball:
            print("Loading classifier head weights")
            classifier_head.load_state_dict(tarball['classifier_state_dict'])

        if 'optimizer' in tarball:
            print("Loading optimizer")
            optimizer.load_state_dict(tarball['optimizer'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss_T, acc1_T, acc5_T = train(train_loader, model, classifier_head, criterion, optimizer, scheduler, epoch, args)

        # evaluate on validation set
        acc1_V, acc5_V = validate(val_loader, model, classifier_head, criterion, args)
        # acc1_R, acc5_R = validate_imagenet_r(val_loader_imagenet_r, classifier_head, model, criterion, args)
        ...
        with open(log_path, "a") as f:
            f.write(f"{epoch},{loss_T},{acc1_T},{acc5_T},{acc1_V},{acc5_V},,\n")


def train(train_loader, model, classifier_head, criterion, optimizer, scheduler, epoch, args):
...
def validate(val_loader, model, classifier_head, criterion, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
...
# def validate_imagenet_r(val_loader, model, classifier_head, criterion, args):
#     batch_time = AverageMeter('Time', ':6.3f')
#     losses = AverageMeter('Loss', ':.4e')
#     top1 = AverageMeter('Acc@1', ':6.2f')
#     top5 = AverageMeter('Acc@5', ':6.2f')
